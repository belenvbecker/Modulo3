...
SELECT * -- V.IdSucursal, sum(v.Precio * v.Cantidad) AS totalVentas
    ...
    INNER JOIN CLIENTE C ON (c.idcliente=v.idcliente)
    INNER JOIN localidad l2 ON (c.IdLocalidad = l2.IdLocalidad) -- localidad del cliente
    WHERE year(Fecha) = 2019 and  l.idprovincia != l2.idprovincia AND L.IDPROVINCIA > 1
  --  GROUP BY v.IDSUCURSAL 
    ORDER BY totalventas DESC;
...
From		
        (select *
		from 
			(select A.idcliente as idcliente, A.Canal,
			row_number()over(partition by idcliente order by idcliente) as Cantidad_canales
			from(
						Select v.idcliente as idcliente ,cv.canal as canal 
						from venta v
						join canal_venta cv
						on v.IdCanal = cv.IdCanal
						where year(v.fecha) in (2019,2020))A
			group by A.idcliente,A.Canal) B
		where canal ='presencial'
		having cantidad_canales=1) C;
        
        
        SELECT Canal, COUNT( DISTINCT IdCliente) FROM venta v
JOIN canal_venta c ON v.IdCanal=c.IdCanal
WHERE YEAR(v.Fecha) BETWEEN 2019 AND 2020
GROUP BY canal;
